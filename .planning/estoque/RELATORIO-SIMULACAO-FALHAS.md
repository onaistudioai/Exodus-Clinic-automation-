...
| 5 | Push diário de alertas (ruptura/validade/saldo negativo) | 🟡 M5 | médio | n8n/cron | ✅ workflow n8n de alertas ativo (cron 08:00 → WAHA) |
...
> **Status (2026-06-21):** A1, M1, M2, M3, M4, M5 corrigidos e em produção. Migration `004-falhas-fixes.sql` aplicada no PG 18.4; painel redeployado e verificado (`/login`=200, rotas protegidas=307, 404 ok; reconciliação = 0 divergências — estoque Aurora vazio pós-cleanup de QA). **M5**: workflow n8n "SOFIA - Alerta Estoque Diário" (o workflow de alertas) ativo — cron `0 8 * * *` → Postgres (digest cross-tenant por superuser; só clínicas COM alertas, sem spam) → WAHA `sendText` p/ `clinicas.telefone_responsavel`. Trigger manual de teste: `POST /webhook/estoque-alerta-trigger` (exige sessão WAHA WORKING + dados de estoque).
>
